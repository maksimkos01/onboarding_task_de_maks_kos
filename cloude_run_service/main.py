...
def consume_batch(country: str, batch_size: int = 100, timeout: float = 15.0):
    if country.lower() not in ['us', 'br']:
        raise HTTPException(status_code=400, detail="Invalid country code. Use 'us' or 'br'.")
    ...
    try:
        ...
        if not messages:
            return {"status": "success", "message": "No new messages to consume.", "count": 0, "dlt_count": 0}
        ...
        for msg in messages:
            ...
            try:
                ...
                if country.lower() == 'us':
                    ...
                    validated_data = [BRSalesElement(**item).model_dump() for item in raw_data]
                ...
                # Publish to the DLT
                error_message = str(e)
                logging.warning(f"Validation failed for message offset {msg.offset()}: {error_message}. Routing to DLT.")
